...
export const updatePromotion = async (req, res) => {
  try {
    ...
    promotion.newPrice = product.currentPrice * (1 - discountPercentage / 100);
...
export const getPromotionsByProduct = async (req, res) => {
  try {
    ...
    const promotions = await Promotion.find({ productId }).populate('productId');

    if (promotions.length === 0) {
      return res.status(404).json({ message: 'No promotions found for this product' });
    }
