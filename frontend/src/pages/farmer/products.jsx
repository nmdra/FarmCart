  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const { data } = await axios.get(`/shops/${shopId}/products`, config);
        setProducts(data);
      } catch (error) {
  ...
  const handleDeleteProduct = async (productID) => {
    if (window.confirm('Are you sure you want to delete this product?')) {
      try {
        ...
        const response = await axios.delete(`/shops/${shopId}/products/${productID}`, config);
        ...
        <div className="grid grid-cols-5 gap-x-1 gap-y-8">
          ...
          {products.map((product) => (
            ...
            >
              ...
              <div className="h-1/2">
                <img src={vege} alt="Product" className="object-cover h-full w-full" />
              </div>

              {/* Lower half - Product details */}
              <div className="h-1/2 p-2 bg-white">
                <h2 className="text-lg font-semibold text-gray-800 text-left">{product.name}</h2>
                <h4 className="text-sm text-gray-600 text-left">{product.description}</h4>
                <h4 className="text-sm font-semibold text-gray-600 text-left">RS. {product.pricePerKg} - 1Kg</h4>
          ...
          <div className="relative flex items-center justify-center bg-white rounded-lg shadow-md h-65 w-60">
            <div className="absolute inset-0 flex flex-col justify-center items-center p-4">
              <img src={addShopIcon} alt="Add Product" className="w-32 h-32 mb-4" />
              <button
