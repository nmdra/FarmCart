...
const CreateOrderForm = () => {
  ...
  const [customerAddress, setCustomerAddress] = useState({ streetAddress: '', city: '', zipCode: '', district: '' });
  const [shopName, setShopName] = useState('');
  const [shopAddress, setShopAddress] = useState({ houseNo: '', streetName: '', city: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:3000/api/od/a', newOrder); 
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-lg mx-auto bg-white shadow-md rounded-lg p-8 space-y-4">
      <h2 className="text-2xl font-bold mb-4 text-gray-800">Create a New Order</h2>
      ...
      <div className="mb-4">
        ...
        <div className="space-y-2">
          <input
            ...
            onChange={(e) => setCustomerAddress({ ...customerAddress, streetAddress: e.target.value })}
            className="mt-1 p-2 w-full border border-gray-300 rounded-md"
          />
          <input
            type="text"
      ...
      <div className="mb-4">
        ...
        <div className="space-y-2">
          <input
            ...
            onChange={(e) => setShopAddress({ ...shopAddress, houseNo: e.target.value })}
            className="mt-1 p-2 w-full border border-gray-300 rounded-md"
          />
          <input
            type="text"
            ...
            onChange={(e) => setShopAddress({ ...shopAddress, streetName: e.target.value })}
            className="p-2 w-full border border-gray-300 rounded-md"
          />
          <input
            type="text"
            ...
            onChange={(e) => setShopAddress({ ...shopAddress, city: e.target.value })}
            className="p-2 w-full border border-gray-300 rounded-md"
          />
        </div>
      </div>
      ...
      {/* Deliver Name */}
      <div className="mb-4">
        <label htmlFor="deliverName" className="block text-sm font-medium text-gray-700">Deliver Name</label>
        <input
          id="deliverName"
          type="text"
          value={deliverName}
