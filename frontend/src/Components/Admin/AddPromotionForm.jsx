...
const AddPromotionPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const discountedPrice = (formData.originalPrice - (formData.originalPrice * formData.discountPercentage / 100)).toFixed(2);

      // Update the formData with the calculated discounted price
      const updatedFormData = { ...formData, discountedPrice };

      const response = await axios.post('/api/Promotion/promotions', updatedFormData);
